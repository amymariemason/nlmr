...
create_data <- function(N, beta1=1.5, beta2=0.5, confound=0.8, errorvar=1) {
# generate G  
...
data$quadratic.Y<-beta2*(data$X)^2+beta1*data$X+confound*data$u+data$errorY
data$sqrt.Y<-beta1*sqrt(data$X)+confound*data$u+data$errorY
data$log.Y<-beta1*log(data$X)+confound*data$u+data$errorY
data$threshold.Y<-ifelse(data$X>beta2,beta1*data$X,0)+confound*data$u+data$errorY
return(data)
}
...
summary_function <- function(data, gene, exposure, outcome, quant=100) { # linear model of G->Y
...
# this divides into strata based on IV-free exposure
  ...
  for (j in 1:length(qs)) {
    BetaYG[j]   = summary(lm(data[quantx0==j,outcome]~data[quantx0==j,gene]))$coef[2]
    seBetaYG[j] = summary(lm(data[quantx0==j,outcome]~data[quantx0==j,gene]))$coef[2,2]
    BetaXG[j]   = summary(lm(data[quantx0==j,exposure]~data[quantx0==j,gene]))$coef[2]
    seBetaXG[j] = summary(lm(data[quantx0==j,exposure]~data[quantx0==j,gene]))$coef[2,2]
    meanX[j] = mean(data[quantx0==j,exposure])
  }
  

 output<-data.frame(BetaXG, BetaYG, seBetaXG, seBetaYG, meanX)
# quants<-data.frame(quantx0)
 print(list(summary=head(output), quantilesort=head(quantx0)))
 invisible(list(summary=output, quantilesort=quantx0))
}
...
# (residual phenotype  = individual's phenotype - centred genetic contribution to phenotype from included genetic variants)
# this means we compare individuals in the population who would have similar phenotype value IF they had the same genetic code
 # see https://www.bmj.com/content/364/bmj.l1042 for details of how this was done with BMI
...
create_summary_data<-function(Ytype="linear", quantiles=100, keep=FALSE, N=10000, beta1=1.5, beta2=0.5, confound=0.8, errorvar=1) {
  ...
  # create the data
    ...
    summ<-summary_function(data, gene="g", exposure="X", outcome=Ytype_name, quant=quantiles)
    summ_data<-summ$summary
    
     # keep entire set if keep variable set to TRUE
   if(keep==TRUE){
     data$quantiles<-summ$quantilesort
      print(paste(N, "individuals generated and summerized into ", quantiles, " quantiles"))
...
lotsofdata<- create_summary_data(Ytype="quad", keep=TRUE, N=100000, beta1=1.5, beta2=0.5, quantiles=100, confound=0)
...
library(ggplot2)
# by genetic type
ggplot(data=alldata, aes(x=X, y=linear.Y) )+ geom_jitter(alpha=0.3, aes(colour=as.factor(g)))+facet_wrap(facets=as.factor(alldata$g))
...
lotsofdata<- create_summary_data(Ytype="quad", keep=TRUE, N=10000, beta1=1.5, beta2=7, quantiles=20, confound=0, errorvar=0)
testdata<-lotsofdata$summary 
alldata<-lotsofdata$alldata
...
fracpoly_mr(alldata$quadratic.Y, alldata$X, alldata$g, family="gaussian", q=10, d=1, fig=T)
