# will generate the degree 2 outcome of nlmr frac poly testing for specific setting
# author: Amy Mason
# input:  par1, par2, beta1, beta2
# input: 
...
par2<- as.numeric(Sys.getenv('par2'))
beta2<-as.numeric(Sys.getenv('beta2'))
...
all<-  foreach(i=1:5,
               .inorder=FALSE,
               .packages = c("nlmr","metafor")) %dorng% {
                 source("../4_assess_output.R")  
                 checkEst(x=i,deg=2, par1=par1,par2=par2,beta0=0,beta1=beta1,beta2=beta2)
...
# p1, p2 in c(-3,-2,-1,-0.5,0,0.5,1,2,3))

    Answer_deg2<-c("all data", nrow(test1_all),test1_all[1, c("p1", "p2", "beta1", "beta2")],
                   mean(test1_all$Beta_1_est), sd(test1_all$Beta_1_est), mean(test1_all$Beta_1_se),
                   mean(test1_all$Beta_2_est), sd(test1_all$Beta_2_est), mean(test1_all$Beta_2_se),
                   mean(test1_all$Beta_1_cov), mean(test1_all$Beta_2_cov),
                   mean(test1_all$powers_correct), mean(test1_all$powers_in_set)
    )
    
    names(Answer_deg2)<-c("type", "simulations", "p1", "p2", "Beta1", "Beta2",
    ...
    Answer_deg2_summ<-c("summerised",nrow(test1_summ),test1_summ[1, c("p1", "p2", "beta1", "beta2")],
                        mean(test1_summ$Beta_1_est), sd(test1_summ$Beta_1_est), mean(test1_summ$Beta_1_se),
                        mean(test1_summ$Beta_2_est), sd(test1_summ$Beta_2_est), mean(test1_summ$Beta_2_se),
                        mean(test1_summ$Beta_1_cov), mean(test1_summ$Beta_2_cov),
                        mean(test1_summ$powers_correct), mean(test1_summ$powers_in_set)
    )
    
    names(Answer_deg2_summ)<-c("type","simulations", "p1", "p2", "Beta1", "Beta2",
                               "est_Beta1_mean", "est_Beta1_sd", "est_Beta1_mean_SE",
                               "est_Beta2_mean", "est_Beta2_sd", "est_Beta2_mean_SE",
                               "cov_beta1", "cov_beta2",
                               "fit_correct", "fit_set"
    ...
    Ans_deg2<-rbind(Answer_deg2, Answer_deg2_summ)
    Ans_deg2_b<-rbind(c(max_diffs, mean_diffs, se_diffs, sig_changes), 
                      c(max_diffs, mean_diffs, se_diffs, sig_changes))
    Ans2<-cbind(Ans_deg2,Ans_deg2_b)
    Ans2<-as.data.frame(Ans2)
    Ans2$date<-as.character(Sys.time())
    Ans2$date2<-Sys.time()
    Ans2<-as.matrix(Ans2)
    ...
    names_out2<- colnames(Ans2)
    
    if (!file.exists("../Table_output/table2.csv")) {  
      write.table(x=t(names_out2), file="../Table_output/table2.csv", col.names=FALSE, row.names=FALSE, sep=",")
      
    }
    
    write.table(x=Ans2, file="../Table_output/table2.csv", append=TRUE, col.names = FALSE, row.names=FALSE, sep=",")
