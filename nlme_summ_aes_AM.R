...
frac_poly_summ_mr_AM <- function(by, bx, byse, bxse, xmean, method="FE", d=1, powers=c(0, -3, -2, -1.5, -1, -0.5, 1, 2), pd=0.05, ci="model_se", nboot=100, fig=FALSE, family="binomial",
...
if (fig==TRUE) {
  results <- list(n=NA, model=model, powers=powers, coefficients=coefficients, lace=lace, xcoef=xcoef_quant, p_tests=p_tests, p_heterogeneity=p_heterogeneity, loglik=loglik, fitted=fitted,figure=figure) }
if (fig==FALSE) {
  results <- list(n=NA, model=model, powers=powers, coefficients=coefficients, lace=lace, xcoef=xcoef_quant, p_tests=p_tests, p_heterogeneity=p_heterogeneity, loglik=loglik,fitted=fitted) }
